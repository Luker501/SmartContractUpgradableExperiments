...
contract PermissionCheck is PermissionCheckAbstract {
	    ...
	    ShareholderRightsDataAbstract RightsCont;
    ...
    constructor(address PermList, address ShareRightsData) public{
        Owner = msg.sender;
        if ((PermList!=address(0)) &&((ShareRightsData!=address(0)))){
            
            Owner = msg.sender;
            PermListCont = PermissionListAbstract(PermList);
            RightsCont = ShareholderRightsDataAbstract(ShareRightsData);
    ...
    function hasOwnershipOfPermissionInstance(address user, uint permInstID, bool shareClass,  uint ShareClassId) public returns (bool){
    	    ...
    	    if (shareClass == true){
    	        ...
    	        uint  fromRights = RightsCont.GetShareholderNumOfSharesOfclass(user, ShareClassId);
    	        ...
    	        if ((fromRights == 0)||(ShareClassId != fromPermList)){
    	            //revert if the shereholder holds no shares of this class 
    	            //or if the given shareholder id given is not actually the owner of the perm
  	                return false;
    	        }
    	    } else {
